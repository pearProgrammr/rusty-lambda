...
fn type_check(node: Term) -> Result<TermType, String> {
    match node {
        ...
        Term::MathOp { opr: _, t1, t2 } => {
            type_check_bin_math_op(type_check(*t1).unwrap(), type_check(*t2).unwrap())
        }
        Term::Equals {
            left_side: t1,
            right_side: t2,
        } => type_check_bin_logic_op(type_check(*t1).unwrap(), type_check(*t2).unwrap()),
        Term::NotEquals {
            left_side: t1,
            right_side: t2,
        } => type_check_bin_logic_op(type_check(*t1).unwrap(), type_check(*t2).unwrap()),
        Term::IfStmt {
            ...
            type_check(*c).unwrap(),
            type_check(*tb).unwrap(),
            type_check(*eb).unwrap(),
        ),
